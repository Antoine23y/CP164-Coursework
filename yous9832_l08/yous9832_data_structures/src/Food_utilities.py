...
def by_origin(foods, origin):
    ...
    for food_item in foods:
        if food_item.origin == origin:
            origins.append(food_item)
...
def calories_by_origin(foods, origin):
    ...
    # Your code here
    food_list = by_origin(foods, origin)
    avg = average_calories(food_list)
...
def food_table(foods):
    ...
    for food_item in sorted_foods:
        ...
        vegetarian = 'True'
        if food_item.is_vegetarian == 0:
            vegetarian = 'False'

        food_calories = int(food_item.calories)

        print(
            f'{food_item.name:<35} {food_item.origin:<12} {vegetarian:<10} {food_calories:>8}')
...
def food_search(foods, origin, max_cals, is_veg):
    ...
    for food in foods:
        check_origin = origin == -1 or int(food.origin) == origin
        check_calories = max_cals == 0 or int(food.calories) == max_cals
        check_veg = not is_veg or food.is_vegetarian

        if check_origin and check_calories and check_veg:
            result.append(food)
