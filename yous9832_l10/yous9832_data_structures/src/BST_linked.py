...
class BST:
    ...
    def _eq_aux(self, node1, node2):
        ...
        use: equals = self._eq_aux(node1, node2)
        ---------------
        Parameters:
            node1 - a root bst node (_BST_Node)
            node2 - a root bst node to be compared (_BST_Node)
        Returns:
            equals - True if subtree contains the same values
                as target in the same location, otherwise False. (boolean)
        -------------------------------------------------------
        """
        equals = True
    ...
    def _two_child_count_aux(self, node):
        ...
        if node is None:
            ...
            count = self._two_child_count_aux(
                node._left) + self._two_child_count_aux(node._right) + 1
    ...
    def _one_child_count_aux(self, node):
        ...
        ---------------------------------------------------------
        Returns the number of the three types of nodes in a BST.
        Use: count = bst._one_child_count_aux(node)
        -------------------------------------------------------
        Parameters:
            node - a bst node (BST_Node_)
        ...
        if node is None:
            ...
            count = self._one_child_count_aux(
                node._left) + self._one_child_count_aux(node._right) + 1
    ...
    def _is_balanced_aux(self, node):
        ...
        Returns whether a bst is balanced, i.e. the difference in
        height between all the bst's node's left and right subtrees is <= 1.
        Use: b = bst.is_balanced_aux(node)
        ---------------------------------------------------------
        Parameters: 
            node - the root bst node(BST_Node_)
        ...
        if node is None or node._height == 1:
            balanced = True
        elif self._node_height(node._right) - self._node_height(node._left) > 1:
            balanced = False
        else:
    ...
    def _is_valid_aux(self, node):
        ...
        if node is None:
            ...
            valid = False
        elif (node._left is not None and node._height != (self._node_height(node._left) + 1)) or (node._right is not None and node._height != (self._node_height(node._right) + 1)):
            valid = False
        else:
    ...
    def levelorder(self):
        """
        -------------------------------------------------------
        Copies the contents of the tree in level order order to a list.
